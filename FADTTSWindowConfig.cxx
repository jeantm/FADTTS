...
void FADTTSWindowConfig::SyncUiToModelStructure()
{
    ...
    itemList.clear();
    for( unsigned long i = 0; i < para_inputTab_covariates_listWidget->count(); i++ )
    {
    ...
    m_para_m->setpara_inputTab_covariates_listWidget( itemList );
    ...
    m_para_m->setpara_subjectTab_subMatrixFile_checkBox( para_subjectTab_subMatrixFile_checkBox->checkState() );

    m_para_m->setpara_subjectTab_qcThreshold_doubleSpinBox( para_subjectTab_qcThreshold_doubleSpinBox->value() );
...
void FADTTSWindowConfig::SyncModelStructureToUi()
{
    ...
    {
        itemList = m_para_m->getpara_inputTab_covariates_listWidget();
    }
    para_inputTab_covariates_listWidget->clear();
    for( std::map<std::pair<unsigned long,QString>,bool>::iterator it = itemList.begin(); it != itemList.end(); it++ )
    {
        QListWidgetItem *item = new QListWidgetItem( it->first.second , para_inputTab_covariates_listWidget );
    ...
    para_subjectTab_subMatrixFile_checkBox->setChecked( m_para_m->getpara_subjectTab_subMatrixFile_checkBox() );

    para_subjectTab_qcThreshold_doubleSpinBox->setValue( m_para_m->getpara_subjectTab_qcThreshold_doubleSpinBox() );
